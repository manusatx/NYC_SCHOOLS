...
library(lattice)
transparentTheme(pchSize = 1, trans = .7)
mean.values=mean(SchoolMain_new$Average_Proficiency)
histogram(~Average_Proficiency, data = SchoolMain_new, xlab = "Average_Proficiency", 
          panel = function(x,...) {
            panel.histogram(x, ...) 
            panel.abline(v=mean.values, col.line="red", identifier = "abline" ,lty = 4)
            })
...
reduced_top_pls_vars = rownames(pls_school_reduced_imp$importance)[reduced_school_order_pls_index[c(1:3)]]
reduced_top_pls_vars

#Explore the univariate relationships of top 3 variables with Yield using featureplot
