...
SchoolMain <- read.csv("SchoolMain.csv")

#data exploration for predictors
#Remove ID and name and other Identificable varaibles by looking at the data
...
SchoolMain[is.na(SchoolMain$Average_Proficiency),]
...
#Create a df with predictors only and without the factor columns
...
print('From the continuous variables 7 correlated variables were removed with correlation check')
...
print('PC1, PC2 till PC15 explain 90% of variance cumulatively as seen from the summary')
#Plot the resultant Principal components
#biplot(SchoolMain.pca, scale = 1, cex = 0.6)

#with removed highly correlated variables datset is
...
plot(hclust.inputdf)
rect.hclust(hclust.inputdf, k=15, border="red")
#Cut the above into 10 clusters and assign name to data frame
groups.inputdf <- as.data.frame(cutree(hclust.inputdf, k=15))
library(data.table)
...
#Apply function r2clusterfun
SchoolMain_subset<- r2clusterfun(SchoolMain_new_numeric,insim='spearman')
SchoolMain_subset
...
#Create a data set with reduced variables
...
plot(pls_school_imp, top=15, scales = list(y = list(cex = 0.8)))
school_order_pls_index <- order(abs(pls_school_imp$importance),decreasing=TRUE)
top_pls_vars = rownames(pls_school_imp$importance)[school_order_pls_index[c(1:5)]]
top_pls_vars
...
plot(pcr_school_imp, top=15, scales = list(y = list(cex = 0.8)))
...
rf_school_model = train( Average_Proficiency ~., data = trainschool,method="cforest",
                         trControl=trainControl(method="repeatedcv", number = 10),
...
```
```{r}
#Run the boosted linear model
library(bst)
blm_school_model = train( Average_Proficiency ~., data = trainschool,method="BstLm",
                         trControl=trainControl(method="repeatedcv", number = 10),
                         preProc = c("center", "scale"), tuneLength = 20)
summary(blm_school_model)
head(blm_school_model$results)
```
...
resamp_school = resamples( list(svm=svm_school_model,ridge=ridge_school_model,lasso=lasso_school_model,enet=enet_school_model) )
...
r2_pls = cor(Average_Proficiency_pls_reduced_hat,testreducedschool$Average_Proficiency,method="pearson")^2
r2_pls
rmse_pls = sqrt( mean( (Average_Proficiency_pls_reduced_hat-testreducedschool$Average_Proficiency)^2 ) )
rmse_pls
```